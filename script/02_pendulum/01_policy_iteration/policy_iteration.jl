...
function policy_evaluation!(agent::Agent)
...
function policy_improvement!(agent::Agent)
    policy=agent.policy_table
    value=agent.value_table
    actions=agent.env.actions
    gamma=agent.discount_factor
...
function policy_iteration!(agent::Agent)
    for j in 1:20
        for i in 1:20
...
gif(anim, "inverted_pendulum.gif", fps = 50)