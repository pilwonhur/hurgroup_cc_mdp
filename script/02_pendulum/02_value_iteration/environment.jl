...
# Define the pendulum environment in Julia
mutable struct Environment
    params::Dict{Symbol, Float64} # g, l, m, nGridθ, nGridω, nActions, ω_min, ω_max, θ_min, θ_max
    θ_space::Vector{Float64}
    ω_space::Vector{Float64}
    terminal_states::Array{Tuple{Int, Int}, 1}
    rewards::Dict{Tuple{Int, Int}, Float64}
    transitions::Dict{Tuple{Int, Int, Int}, Dict{Tuple{Int, Int}, Float64}} # (current_state, action) -> (new_state, prob for actions)
    actions::Vector{Int}
end

# Initialize a new GridWorld environment
function Environment(params, terminal_states, rewards, actions)
    θ_min, θ_max, θ_steps = params[:θ_min], params[:θ_max], params[:nGridθ]+1
    ω_min, ω_max, ω_steps = params[:ω_min], params[:ω_max], params[:nGridω]+1
    θ_space = range(θ_min, θ_max, length=θ_steps)
    ω_space = range(ω_min, ω_max, length=ω_steps)
    transitions = Dict{Tuple{Int, Int, Int}, Dict{Tuple{Int, Int}, Float64}}()
    return Environment(params, θ_space, ω_space, terminal_states, rewards, transitions, actions)
end
